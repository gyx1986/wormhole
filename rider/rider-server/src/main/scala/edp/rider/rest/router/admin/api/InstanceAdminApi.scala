              ...
              if (session.roleType == "user") {
                ...
                if (namePattern.matcher(simple.nsInstance).matches()) {
                  if (checkSys(simple.nsSys) && checkFormat(simple.nsSys, simple.connUrl)) {
                    val instance = Instance(0, simple.nsInstance.trim, simple.desc, simple.nsSys.trim, simple.connUrl.trim,Some(simple.version.trim), active = true, currentSec, session.userId, currentSec, session.userId)
              ...
              if (session.roleType != "admin") {
                ...
                if (namePattern.matcher(instance.nsInstance).matches()) {
                  if (checkFormat(instance.nsSys, instance.connUrl)) {
                    val instanceUpdate = Instance(instance.id, instance.nsInstance.trim, instance.desc, instance.nsSys.trim, instance.connUrl.trim, instance.version,instance.active, instance.createTime, instance.createBy, currentSec, session.userId)

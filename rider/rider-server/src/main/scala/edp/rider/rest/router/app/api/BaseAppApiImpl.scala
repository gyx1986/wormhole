...
class BaseAppApiImpl[T <: BaseTable[A], A <: BaseEntity](baseDal: BaseDal[T, A]) extends BaseRoutesApi with RiderLogger with JsonSerializer {
  ...
  private def generateEntity(base: BaseEntity, session: SessionClass): BaseEntity = {
    base match {
      case instance: Instance => Instance(instance.id, instance.nsInstance, Some(instance.desc.getOrElse("")), instance.nsSys, instance.connUrl, Some(instance.version.getOrElse("")),instance.active, instance.createTime, instance.createBy, currentSec, session.userId)
